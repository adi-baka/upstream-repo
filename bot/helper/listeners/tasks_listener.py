...
class MirrorLeechListener:
    ...
    async def onUploadComplete(self, link, size, files, folders, mime_type, name, rclonePath='', dir_id=''):
        ...
        lmsg += f'\n<b>👤 User</b>: {self.tag}'
        gmsg = f'<b>👤 User</b>: {self.tag}'
        msg = f'\n<b>💾 Size</b>: {get_readable_file_size(size)}'
        msg += f"\n<b>⏳ Elapsed</b>: {get_readable_time(time() - self.extra_details['startTime'])}"
        msg += f"\n<b>💠 Mode</b>: {self.extra_details['mode']}"
        _msg = '' if rclonePath == '' else f'\n\n<b>🚩 Path</b>: <code>{rclonePath}</code>'
        msg_ = '\n\n<b><i>🏁 Links Have Been Sent In DM!</i></b>'
        buttons = ButtonMaker()
        if self.isLeech:
            msg += f'\n<b>📂 Total Files</b>: {folders}\n'
            if mime_type != 0:
                msg += f'\n<b>☠️ Corrupted Files</b>: {mime_type}\n'
            msg_ = '\n<b><i>🏁 Files Have Been Sent In DM!</i></b>'
            ...
            if link or rclonePath and config_dict['RCLONE_SERVE_URL']:
                ...
                if link:
                    if link.startswith("https://drive.google.com/") and not config_dict['DISABLE_DRIVE_LINK']:
                        buttons.ubutton("☁️ GDrive Link", link)
                ...
                if rclonePath and (RCLONE_SERVE_URL := config_dict['RCLONE_SERVE_URL']):
                    ...
                    if mime_type == "Folder":
                        share_url += '/'
                    buttons.ubutton("®️ Rclone Link", share_url)
                elif not rclonePath:
                    ...
                    if INDEX_URL:
                        ...
                        if mime_type == "Folder":
                            ...
                            if mime_type.startswith(('image', 'video', 'audio')):
                                ...
                                buttons.ubutton("👁️‍🗨️ View", share_urls)
                ...
                if self.logMessage:
                    if link.startswith("https://drive.google.com/") and config_dict['DISABLE_DRIVE_LINK']:
                        buttons.ubutton("☁️ GDrive Link", link, 'header')
    ...
    async def onDownloadError(self, error, button=None):
        async with download_dict_lock:
            ...
            if self.sameDir and self.uid in self.sameDir['tasks']:
                self.sameDir['tasks'].remove(self.uid)
                self.sameDir['total'] -= 1
        msg = f"<b>👤 User:</b> {self.tag}!\n<b>⚠️ Your Task Has Been Stopped!</b>"
        msg += f"\n\n<b>ℹ️ Reason</b>: <code>{escape(str(error))}</code>"
        msg += f"\n<b>⏳ Elapsed</b>: <code>{get_readable_time(time() - self.extra_details['startTime'])}</code>"
        msg += f"\n<b>💠 Mode</b>: <code>{self.extra_details['mode']}</code>"
